...
gm_ = on_command("神秘力量", permission=SUPERUSER, priority=5)
gmm_ = on_command("未知力量", permission=SUPERUSER, priority=5)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if stone_num:
        ...
        if XiuConfig().img:
            msg = await pic_msg_format(msg, event)
            pic = await get_msg_pic(msg)
            await gm_.finish(MessageSegment.image(pic))
        else:
            await gm_.finish(msg, at_sender=True)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if give_user:
        ...
        if XiuConfig().img:
            msg = await pic_msg_format(msg, event)
            pic = await get_msg_pic(msg)
            await gmm_.finish(MessageSegment.image(pic))
        else:
            await gmm_.finish(msg, at_sender=True)
    else:
        msg = "对方未踏入修仙界，不可修改！"
        if XiuConfig().img:
            msg = await pic_msg_format(msg, event)
