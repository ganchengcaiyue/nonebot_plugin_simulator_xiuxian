...
async def get_boss_battle_info(user_info, rift_rank, bot_id):
    ...
    if sql_message.xito() == 0:
        ...
        result = f"看什么看"
        msg += f"(系统代打)获得了修为：{give_exp}点，灵石：{give_stone}枚！"
    else:
...
def get_dxsj_info(rift_type, user_info):
    ...
    if sql_message.xito() == 0:
        cost_stone = 260000
        sql_message.update_ls(user_info.user_id, cost_stone, 1)
        msg = random.choice(battle_data[rift_type]['desc']).format(f"系统驱散雾气并击杀了敌人，获得灵石：{cost_stone}枚！")
    else:
...
def get_treasure_info(user_info, rift_rank):
    ...
    if rift_type == "法器":
        if sql_message.xito == 0 :
            temp_msg = f"(系统)竟然获得了上仙法器：相云剑！"
            msg = random.choice(TREASUREMSG).format(temp_msg)
            sql_message.send_back(user_info.user_id, 7094, "相云剑", "装备", 1)
        else:
        ...
        if sql_message.xito == 0 :
            temp_msg = f"(系统)竟然获得了上仙防具：歆妄袍！"
            msg = random.choice(TREASUREMSG).format(temp_msg)
            sql_message.send_back(user_info.user_id, 6092, "歆妄袍", "装备", 1)
        else:
        ...
        if sql_message.xito == 0 :
            temp_msg = f"(系统)竟然获得了仙阶上品功法：化气清天录！"
            msg = random.choice(TREASUREMSG).format(temp_msg)
            sql_message.send_back(user_info.user_id, 9913, '化气清天录', "技能", 1)
        else:
        ...
        if sql_message.xito() == 0:
                temp_msg = f"(系统)竟然获得了仙阶上品神通：混日剑法！"
                msg = random.choice(TREASUREMSG).format(temp_msg)
                sql_message.send_back(user_info.user_id, 8912, "混日剑法", "技能", 1)
        else:
            ...
            if give_sec_info[0]:#获得了
                ...
                temp_msg = f"竟然获得了{sec_buff['level']}神通：{sec_buff['name']}！"
                msg = random.choice(TREASUREMSG).format(temp_msg)
        ...
        if sql_message.xito() == 0:
            ...
            temp_msg = f"(系统)竟然获得了灵石：{give_stone}枚！"
            msg = random.choice(TREASUREMSG).format(temp_msg)
        else:
